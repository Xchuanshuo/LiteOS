...
int32_t sys_unlink(const char* pathname) {
    ...
    while (file_idx < MAX_FILE_OPEN) {
        if (file_table[file_idx].fd_inode != NULL &&
            inode_no == file_table[file_idx].fd_inode->i_no) {
            break;
        }
        file_idx++;
    }
...
int32_t sys_mkdir(const char* pathname) {
    uint8_t rollback_step = 0; // 用与操作失败时回滚各个资源状态
    ...
    if (inode_no != -1) { // 如果找到了同名目录或文件
        ...
        uint32_t pathname_depth = (uint32_t) path_depth_cnt((char*) pathname);
        uint32_t path_search_depth = (uint32_t) path_depth_cnt(searched_record.searched_path);
        // 先判断是否把pathname的各层目录都访问到了,即是否存在某个中间目录查找失败了
...
/** 读取目录dir的1个目录项,成功后返回其目录项地址,到目录尾或出错是返回NULL */
...
int32_t sys_chdir(const char* path) {
   ...
   if (inode_no != -1) {
       if (searched_record.file_type == FT_DIRECTORY) {
           running_thread()->cwd_inode_no = (uint32_t) inode_no;
           ret = 0;
       } else {
