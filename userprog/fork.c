...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread,
    ...
    // 此时child_thread->userprog_vaddr.vaddr_bitmap.bits还是指向父进程虚拟地址
    // 的位图地址,下面指向自己的位图vaddr_btmp
        ...
        if (vaddr_btmp[idx_byte]) {
            ...
            while (idx_bit < 8) {
                if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte]) {
                    ...
                    // a.将父进程用户空间中的数据复制到内核缓冲区buf_page,目的是切换
                    // 到子进程的页表后,还能访问到父进程的数据
                    memcpy(buf_page, (void*) prog_vaddr, PG_SIZE);
                    // b.将页表切换到子进程,目的是避免下面申请内存的函数将
                    // pte及pde安装在父进程的页表中
                    page_dir_activate(child_thread);
                    // c.申请虚拟地址prog_vaddr
                    get_a_page_without_opvaddrbitmap(PF_USER, prog_vaddr);
                    // d.从内核缓冲区将父进程的数据复制到子进程的用户空间
...
static int32_t build_child_stack(struct task_struct* child_thread) {
    ...
    // 修改子进程的返回值为0
    intr_0_stack->eax = 0;
    /** b.为switch_to构建thread_stack,将其构建在紧临intr_stack之下的空间 */
