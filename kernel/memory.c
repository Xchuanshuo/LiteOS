...
/************ 位图地址 ***************
 *  因为0xc009f00是内核主线程栈顶,0xc009e00是内核主线程的pcb.
 *  一个页框大小的位图可表示128M内存,位图位置安排在地址0xc009a00,
...
/* 0xc0000000是内核从虚地址3G起. 0x100000指跨过低端1M内存,使虚拟地址在逻辑上连续 */
...
struct arena {
    ...
    // large为true时,cnt表示的是页数
    // 否则cnt表示空闲mem_block的数量
    uint32_t cnt;
    bool large;
};

struct mem_block_desc k_block_descs[DESC_CNT]; // 内核内存块描述符
...
void* malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
     ...
     * 3.通过page_table_add将以上得到的虚拟内存地址和物理地址在页表中完成映射 */
    ...
    // 因为虚拟地址是连续的,但物理地址可以不是连续的,所以逐个映射
    while (cnt-- > 0) {
...
uint32_t addr_v2p(uint32_t vaddr) {
    uint32_t* pte = pte_ptr(vaddr);
    // 取pte的高20位就是物理页的起始地址,然后加上虚拟地址的低12位
    // 最终得到的就是物理地址
...
void* sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        if (a != NULL) {
            ...
            // 对于分配的大块页框,将desc置为NULL,cnt置为页框数,large为true
            a->desc = NULL;
            a->cnt = pg_cnt;
            a->large = true;
            lock_release(&mem_pool->lock);
    ...
    } else { // 若申请的内存小于1024字节,则在各种规格的mem_block_desc中去适配
        uint32_t desc_idx;
        // 从内存块描述符中匹配合适的内存规格块
        ...
        // 当mem_block_desc的free_list没有可用的mem_block
        // 就创建新的arena提供mem_block
        if (list_empty(&descs[desc_idx].free_list)) {
            a = malloc_page(PF, 1); // 分配1页内存作为arena
            if (a == NULL) {
                lock_release(&mem_pool->lock);
                return NULL;
            }
...
void pfree(uint32_t pg_phy_addr) {
    struct pool* mem_pool;
    uint32_t bit_idx = 0;
    if (pg_phy_addr >= user_pool.phy_addr_start) { // 用户物理池
        mem_pool = &user_pool;
...
void sys_free(void* ptr) {
    ...
    if (ptr != NULL) {
        ...
        if (a->desc == NULL && a->large == true) { // 大于1024字节
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    // 已使用内存: 页表大小 + 0x100000低端1M内存
    uint32_t used_mem = page_table_size + 0x100000;
    uint32_t free_mem = all_mem - used_mem;
    // 1页为4k,不管总内存是不是4k的倍数 对于以页为单位的内存分配策略，不足1页的内存不用考虑了
    uint16_t all_free_pages = (uint16_t) (free_mem / PG_SIZE);
    // 内核空闲页
    uint16_t kernel_free_pages = (uint16_t) (all_free_pages / 2);
    // 用户空闲页
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;

    // 为简化位图操作,余数不做处理,坏处是这样会丢掉内存
    // 好处是不用做内存的越界检查,因为位图表示的内存少于实际物理内存
    uint32_t kbm_length = (uint32_t) (kernel_free_pages / 8); // Kernel BitMap的长度,位图中的一位表示一页,以字节为单位
    uint32_t ubm_length = (uint32_t) (user_free_pages / 8);   // User BitMap的长度.
 ...
 *   ************************************************/
    ...
    /* 初始化虚拟地址的位图,按实际物理内存大小生成数组 */
    // 用于维护内核堆的虚拟地址,所以要和内核内存池大小一致
    kernel_vaddr.vaddr_bitmap.btmp_bytes_len = kbm_length;
    // 位图的数组指向一块未使用的内存,目前定位在内核物理内存池和用户物理内存池之后
...
/** 根据物理页框地址pg_phy_addr将相应内存的位图清0,不改动页表 */
