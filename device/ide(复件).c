...
/** 一些硬盘操作的指令 */
#define CMD_IDENTITY 0xec      // identify指令
...
// 用于记录总扩展分区起始lba,初始化为0,partition_scan时以此为标记
int32_t ext_lba_base;
// 用于记录硬盘主分区和逻辑分区的下标
...
struct partition_table_entry {
    ...
    uint32_t start_lba;        // 本分区的起始扇区的lba地址
...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ...
    // 写入lba地址
    outb(reg_lba_l(channel), lba); // la地址的低8位
    outb(reg_lba_m(channel), lba >> 8); // lba地址的8~15位
    outb(reg_lba_h(channel), lba >> 16); // lba地址的16~23位

    // lba地址的24~27为要存储在device寄存器的0~3位
    // 无法单独写入这4位,所以在此处把device寄存器再重写入一次
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    while (secs_done < sec_cnt) {
        ...
        // 在硬盘已经开始工作(开始在内部读/写数据)后才能阻塞自己,等待硬盘
        // 完成读操作后通过中断处理程序唤醒自己
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
    ...
    while (part_idx++ < 4) {
        if (p->fs_type == 0x5) { // 若为扩展分区
            if (ext_lba_base != 0) {
                // 子扩展分区的start_lba相当于主引导扇区的总扩展分区地址
                partition_scan(hd, p->start_lba + ext_lba_base);
            } else {
                // ext_lba_base为0表示是第一次读取引导块,也就是主引导记录所在的扇区
                // 记录下扩展分区的起始lba地址,后面所有额扩展都相对与此
    ...
    while (channel_no < channel_cnt) {
        ...
        switch (channel_no) {
            case 0:
                ...
                // 从片8259a上倒数第二的中断引脚,也就是ide0通道的的中断向量号
                channel->irq_no = 0x20 + 14;
                break;
            case 1:
                // ide1通道的起始端口号是0x170
                channel->port_base = 0x170;
                // 从片8259A上的最后一个中断引脚,我们用来响应ide1通道上的硬盘中断
