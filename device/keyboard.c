...
static void intr_keyboard_handler(void) {
    ...
    bool shit_down_last = shift_status;
    ...
    // 看是否是break_code, 断码第8位为1
    break_code = ((scancode & 0x0080) != 0);
    if (break_code) {
        // 由于ctrl_r、alt_r的make_code和break_code都是两字节
        // 所以用以下方式取make_code, 多字节的扫描码暂不处理
        uint16_t make_code = (scancode &= 0xff7f);
        // 若是任意以下三个键弹起了,则将状态设为false
        ...
        if (scancode < 0x0e || scancode == 0x29 ||
            ...
            /****** 代表两个字母的键 ********
                  ...
                  0x35 字符'/'
            *******************************/
            if (shit_down_last) {    // 如果同时按下了shift键
                shift = true;
            }
        } else {
            // 默认为字母键
            if (shit_down_last && caps_lock_last) {
                // 如果shift和capslock同时按下
                shift = false;
            } else if (shit_down_last || caps_lock_last) {
                shift = true;
            } else {
                shift = false;
            }
